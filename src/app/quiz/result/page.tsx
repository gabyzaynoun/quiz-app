...
// simple pet gear map (swap hrefs later)
const PET_PRODUCTS: Record<"dog" | "cat" | "multi", { id: string; title: string; subtitle?: string; href: string }[]> = {
  dog: [
...
export default function ResultPage() {
  ...
  // locate the pet question's answer
  ...
  const { totals, order, percents } = useMemo(() => {
    ...
    return { totals: t, order: ord, percents: p };
  }, [selected]);

  const topKey = order[0] ?? "owl";
  const runnerUpKey = order[1] ?? "fox";

  const main = QUIZ.results.find(r => r.weightKey === topKey)!;
  const runner = QUIZ.results.find(r => r.weightKey === runnerUpKey)!;
  ...
  return (
    <main className="container space-y-6">
      ...
      {petKind !== "none" && (
        <div className="card">
          <div className="card-body">
            <h3 className="text-lg font-semibold">Recommended for your {petKind === "multi" ? "pets" : petKind}</h3>
            <div className="mt-3 grid sm:grid-cols-2 gap-3">
