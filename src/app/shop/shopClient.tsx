...
export default function ShopClient() {
  ...
  const animal = (search.get("type") as AnimalKey) || "owl";
  const pet = (search.get("pet") as "dog" | "cat" | "multi" | "none") || "none";

  const base = ANIMAL_TO_AUDIENCE[animal] ?? ["focus"];
  const petKey: AudienceKey | null =
    pet === "dog" ? "dogs" : pet === "cat" ? "cats" : pet === "multi" ? "multi" : null;

  const audiences = petKey ? [...base, petKey] : base;
  const items = byAudience(audiences);

  const meta = QUIZ.results.find((r) => r.weightKey === animal) ?? QUIZ.results[0];
    ...
    <main className="container space-y-6">
      <div className="card" style={{ boxShadow: "var(--shadow-soft)" }}>
        <div className="card-body">
          ...
          <h1 className="text-3xl font-extrabold mt-1">Gear for {meta.label}</h1>
